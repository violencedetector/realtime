...
##################################################################################
    @staticmethod
    def return_points(npimg, humans, imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
        centers = {}
...
##                    print("if")
                #    continue
                    ...
                    centers[i] = center
                #cv2.circle(npimg, center, 3, common.CocoColors[i], thickness=3, lineType=8, shift=0)
                    points[human.body_parts[i].part_idx]=center#.uidx)#center)#
##                    print(center)
##                    print(human.body_parts[i].part_idx)
                else:
                    points[i]=(0,0)
            #points=[]#
            # draw line
            ...
            #        continue
                ...
                #cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
            points_copy=points[:]
            individuals.append(points_copy)#
##            print("append individuo")
##            print(individuals)
            #for clean in range(18):
            #    points[clean]=(0,0)#points=[]#
##        print("final")
##        print(individuals)
        return individuals#
##################################################################################
